...
- Домен: OrderService, реализующий бизнес-правила применения скидки и расчёта итоговой суммы (ссылайся на `backend/skills/api-design.md`).
- Инфраструктура: OrderRepositoryImpl для PostgreSQL (с транзакцией).
- Событие: Отправить OrderCreatedEvent в Kafka после успешного коммита транзакции (использовать паттерн Outbox из `backend/skills/async-processing.md`).
...
- Если данных нет в кэше Redis, сходи в Postgres (референс: `backend/skills/database-modeling.md`).
```
